...
O Ditto permite que você escreva estilos para pseudo estados, media queries e atributos de dados personalizados com as propriedades de estilo condicionais.
...
```jsx
<chakra.div bg={{ base: 'white', _osDark: 'black' }}>Olá</chakra.div>
```
...
```jsx
<div className="group">
  <Text _groupHover={{ bg: 'red.500' }}>Passe o mouse</Text>
</div>
```
...
O Ditto permite que você crie suas próprias condições, então você não está limitado às condições no preset padrão. Saiba mais sobre como personalizar condições [aqui](#).
