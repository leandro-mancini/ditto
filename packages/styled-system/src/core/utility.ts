...
export function createUtility(options: Options) {
  ...
  const tokeStylenMap = getTokenMap(tokens);

  const tokenStyleFn: TokenStyleFn = (path: string, fallback?: any) => {
    return tokeStylenMap.get(path)?.value || fallback;
  };

  const transformStyle = memo((prop: string, raw: any) => {
    const key = resolveShorthand(prop);
    ...
    const value = tokenStyleFn(`${config.values}.${raw}`);
    if (!config.transform) {
      return defaultTransform(prop, value ?? raw);
    }
